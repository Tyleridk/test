...
RegisterNetEvent("crp-pharm:DutyB")
AddEventHandler("crp-pharm:DutyB", function()
...
RegisterNetEvent("crp-pharm:shop")
AddEventHandler("crp-pharm:shop", function()
...
RegisterNetEvent('crp-pharm:RequireCheck', function(args)
    if args.RequireItem then
        if QBCore.Functions.HasItem(args.RequireItem) then
            TriggerEvent('crp-pharm:UseItem', args)
            if args.RemoveRequired then
                TriggerServerEvent('crp-pharm:RemoveItem', args.RequireItem)
            end
        else
...
RegisterNetEvent('crp-pharm:UseItem', function(args)
    if not isbusy then
        isbusy = true
        local ScreenColour = ""
        ...
        QBCore.Functions.Progressbar("smoke_joint", "Using..", 1500, false, true, {
            ...
            if args.RemoveItem then
                TriggerServerEvent('crp-pharm:RemoveItem', args.ItemName)
            end
