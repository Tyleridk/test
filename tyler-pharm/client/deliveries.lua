...
RegisterNetEvent('crp-pharm:deliveries:DeliverWeed', function(amount)
...
RegisterNetEvent("crp-pharm:DeliveryMenu")
AddEventHandler("crp-pharm:DeliveryMenu", function()
    ...
    if delmenu ~= nil then
        ...
        TriggerEvent("crp-pharm:deliveries:DeliverWeed", delmenu.amount)
...
function KnockDoor(amount, delitem, delitemamount)
    if QBCore.Functions.HasItem(delitem, delitemamount) then
        ...
        QBCore.Functions.Progressbar('falar_empregada', 'Knocking Door...', 5000, false, true, 
            ...
            TriggerServerEvent('crp-pharm:server:KnockDoor', delitem, delitemamount)
...
RegisterNetEvent("crp-pharm:deliveries:PickUpMenu")
AddEventHandler("crp-pharm:deliveries:PickUpMenu", function()
    ...
    if wwmenu ~= nil then
        ...
        TriggerEvent("crp-pharm:deliveries:PickUpWeed", wwmenu.amount)
    end
end)

RegisterNetEvent('crp-pharm:deliveries:PickUpWeed', function(wwamount)
    if not onPickup then
        TriggerEvent('animations:client:EmoteCommandStart', {"type"})
        QBCore.Functions.Progressbar('falar_empregada', 'Getting Weed Order...', 5000, false, true, 
        {disableMovement = true,disableCarMovement = true,disableMouse = false,disableCombat = true,}, {}, {}, {}, function()
        TriggerEvent('animations:client:EmoteCommandStart', {"c"})
        TriggerServerEvent('crp-pharm:server:pickupordered', wwamount)
...
RegisterNetEvent('crp-pharm:DeliveryCancel', function()
    if onPickup then
        finishedPickup = true
        onPickup = false
        DropOffDone = 0
...
function PickUpWetWeed2(wwamount)
    ...
    QBCore.Functions.Progressbar('falar_empregada', 'Picking Up Some Wet Bud...', 5000, false, true, {disableMovement = true,disableCarMovement = true,disableMouse = false,disableCombat = true,}, {}, {}, {}, function()
        ...
        TriggerServerEvent('crp-pharm:server:PickupFinished', wwamount)
...
RegisterNetEvent("crp-pharm:deliveries:ReceivePayment")
AddEventHandler("crp-pharm:deliveries:ReceivePayment", function()
    if onDuty then
    	QBCore.Functions.TriggerCallback('crp-pharm:server:get:ReceiptChecker', function(HasItems)
    		if HasItems then
                ...
                {animDict = "mp_common",anim = "givetake1_a",flags = 8,}, {}, {}, function()
					TriggerServerEvent('crp-pharm:server:ReceivePayment', job)
				end, function()
